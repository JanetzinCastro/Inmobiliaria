  ...
  //guarda los datos que se capturaron en las cajas de texto del formulario
    ...
    $datos=array();
       ...
       if (isset($_SESSION['cveUsuario'])&&(!empty($_SESSION['cveUsuario']))){//start session
          ...
          if(!empty($_POST['txtTelefono']) && !empty($_POST['txtEmail'])&& !empty($_POST['txtCalle'])&& !empty($_POST['txtMunicipio'])&& !empty($_POST['txtColonia'])&& !empty($_POST['txtMunicipio'])&& !empty($_POST['sltEstado'])&& !empty($_POST['sltPago']) ){
      ...
      //toma los valores de la caja de texto
        ...
        $PAGOCVE=htmlspecialchars($_POST['sltPago']);
            ...
            $datos=$cliente->regPerfil($TELEFONO,$EMAIL,$CALLE,$COLONIA,$MUNICIPIO,$ESTADO,$USUCVE,$PAGOCVE);
    ...
    if((int)$datos[0]["CLAVE"]!=0){
        //envia a la pagina principal de administrar
            echo '<script language="javascript">alert("Se ha registrado tu Perfil correctamente");document.localtion.href="?op=perfil";</script>';
    }
    else{
        //NO encuentra al usuario
        $datos[0]=0;
            echo '<script language="javascript">alert("Error al registrar el perfil");</script>';
