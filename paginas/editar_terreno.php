  ...
  //guarda los datos que se capturaron en las cajas de texto del formulario
    ...
    $datos=array();
       ...
       if (isset($_SESSION['cveUsuario'])&&(!empty($_SESSION['cveUsuario']))){//start session
        ...
        $TIPOCVE=htmlspecialchars($_POST['sltTipo']);
            ...
            $datos=$cliente->editarTerreno($id,$CALLE,$COLONIA,$MUNICIPIO,$ESTADO,$LARGO,$ANCHO,$PRECIO,$DESCRIPCION,$TIPOCVE);
    ...
    if((int)$datos[0]["CLAVE"]!=0){
        //envia a la pagina principal de administrar
            echo '<script language="javascript">alert("Se ha actualizado tu terreno correctamente");document.localtion.href="?op=mis_terrenos";</script>';
    }
    else{
        //NO encuentra al usuario
        $datos[0]=0;
            echo '<script language="javascript">alert("Error al actualizar el terreno");</script>';
    }
